...
<#
.SYNOPSIS
    Runs the IPBanProSDKTests test suite under coverlet and produces a raw + HTML coverage report.

.DESCRIPTION
    PowerShell port of coverage/run.sh. Invokes `dotnet test` with the XPlat Code Coverage
    collector against IPBanProSDKTests/IPBanProSDKTests.csproj, prints a summary derived from the
...
.PARAMETER Filter
    ...
    Example: -Filter "FullyQualifiedName~Crypto".

.EXAMPLE
    .\coverage\run.ps1
    .\coverage\run.ps1 -Filter "FullyQualifiedName~ClientWebSocketTests"
#>
param(
    [string]$Filter = ""
)
...
if (-not $cobertura) {
    Write-Error ">> no coverage.cobertura.xml produced — check that coverlet.collector is referenced in IPBanProSDKTests.csproj"
    exit 1
}
Write-Host ">> raw report: $cobertura" -ForegroundColor Green

# Print a summary using summary.py — fall back to inline parse if python is missing
